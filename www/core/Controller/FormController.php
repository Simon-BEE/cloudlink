...
/**
 * Le FormController permet de gérer les données envoyées en POST, des erreurs à la sécurisation des données avant l'implémentation en bdd.
 * Il permet de vérifier les contraintes attendues et de renvoyer des erreurs en cas de soucis.
 */
...
class FormController
{
    ...
    private function addData(string $field): void
    {
        if (!isset($this->errors[$field])) {
            $this->datas[$field] = $this->verifData($this->postDatas[$field]);
        }
    }
    ...
    private function verifData(string $data): string
    {
        $verified = trim($data);
        $verified = htmlspecialchars($verified);
        $verified = stripslashes($verified);
